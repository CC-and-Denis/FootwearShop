...
methods: {
  async loadMore(direction) {
    ...
    if (this.$props.apiUrl == '/api/getSimilarProducts') {
      ...
      this.productId = document.getElementById('product_id');
      this.productColor = document.getElementById('product_id');

      const parameters = {
        'type': this.productType.value,
        'material': this.productMaterial.value,
        'id': this.productId.value,
        'color': this.productColor.value,
      }
